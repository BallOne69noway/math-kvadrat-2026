...
db.ref('ai_checks').on('value', s => { aiChecks = s.val() || {}; refreshStatusBanner(); });
...
async function verifyWithAI(team, taskNum, volunteerAnswer) {
    const promptText = `
    Ты — ассистент жюри математической олимпиады.
    Проверь ответ команды на корректность математической записи, эквивалентность и опечатки.
    Номер задачи: ${taskNum}
    Введенный ответ команды: "${volunteerAnswer}"

    Верни СТРОГО JSON объект (без лишних слов и без разметки markdown):
    {
        "isCorrect": true/false,
        "confidence": 0.95,
        "reason": "краткое объяснение на русском почему ответ корректен или сомнителей"
    }
    `;
    ...
    try {
        ...
        return { isCorrect: false, confidence: 0, reason: "Не удалось проанализировать через ИИ" };
    }
}
...
async function sendSubmission() {
    ...
    alert("Отправлено и проверено ИИ!");
}
...
function refreshStatusBanner() {
    ...
    if(aiBox && aiData) {
        ...
        const confPercent = Math.round((aiData.confidence || 1) * 100);
        aiVerdictText.style.color = isOk ? "var(--green)" : "var(--red)";
        aiVerdictText.innerText = `${isOk ? "ВЕРНО ✅" : "НЕВЕРНО ❌"} (${confPercent}% уверенности)`;
...
function fullReset() { 
    ...
    if(masterPass === "MASTER2026") {
        if(confirm("ВЫ УВЕРЕНЫ? ДАННЫЕ БУДУТ УДАЛЕНЫ БЕЗВОЗВРАТНО!")) { 
            // Удаляем все результаты из базы Firebase
            db.ref('scores').remove(); 
            db.ref('submissions').remove(); 
            db.ref('penalties').remove(); 
            db.ref('ai_checks').remove();
